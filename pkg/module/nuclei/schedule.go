...
var (
	...
	source = []string{"ip", "http.port", "http.scheme", "ports.port", "timestamp"}

	// maxTargets per batch
	maxTargets = 50000

	// httpTemplates is a list of types for HTTP based templates
	httpTemplates = []string{"http", "headless", "ssl", "websocket"}

	// networkTemplates is a list of types for network based templates
	networkTemplates = []string{"dns", "network", "whois"}
)
...
func (m *Module) Schedule(ctx context.Context, payload types.SchedulerPayload, scheduler types.Scheduler, es *elasticsearch.Client, logger *logrus.Entry) (total uint32, err error) {
	...
	if payload.PreviousModule == "masscan" {
		templateTypes = networkTemplates
	} else if payload.PreviousModule == "httpx" {
		templateTypes = httpTemplates
	} else {
		return 0, fmt.Errorf("unsupported previous module '%s'", payload.PreviousModule)
	}
	...
	g.Go(func() error {
		...
		for result := range results {
			...
			if i == maxTargets {
				if _, err := scheduler.Nuclei(ctx, payload.Scan, targets, templateTypes); err != nil {
	...
	g.Go(func() error {
		...
		// Construct query
		query := "scan:" + payload.Scan + " AND "
		if payload.PreviousModule == "masscan" {
			query += "ports.metadata.task:" + payload.Task
		} else {
			query += "http.metadata.task:" + payload.Task
		}
		...
		for {
			...
			for index, hit := range result.Hits.Hits {
				if payload.PreviousModule == "masscan" {
					...
					// Unmarshal document
					var doc httpx.Result
					if err := json.Unmarshal(hit.Source, &doc); err != nil {
						logger.Errorf("Failed to unmarshal document '%s': %s", hit.ID, err)
						continue
					}

					for _, http := range doc.Http {
						// Send target
						results <- http.Scheme + "://" + doc.IP + ":" + strconv.Itoa(http.Port)
